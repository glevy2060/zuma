...
public class MoveBallsOnBoarding : MonoBehaviour
{
    ...
    public static bool finishPhaseOneFlag = false;
    ...
    public void setOnBallFlag(bool val)
    {
        onBallFlag = val;
    }
    ...
    private void Update()
    {
        if (onBallFlag)
        {
            ...
            if (ballList.Count == 0 & restartFlag == false)
                finishPhaseOneFlag = true;

            if (restartFlag == true)
            {
                finishPhaseOneFlag = false;
                cases = 2;
                Start();
            }
    ...
    private void CreateNewBall(int? cases = 1)
    {
            if(cases > 1)
            {
